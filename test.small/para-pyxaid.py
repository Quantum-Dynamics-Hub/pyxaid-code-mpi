...
#rt = "/homes/daveturner/apps/pyxaid.mpi"
#rt = "/tmp"
rt = scr_dir
...
for i in range(0,nprocs):
   ...
   if myproc == i and os.path.isdir( scr_dir + "/marker.dir" ) :
      print( str(myproc) + " on " + hostname + " is moving files to " + res_dir )
      os.system( "mv -f " + scr_dir + "/[mdo][eu]* " + res_dir )
      os.system( "mv " + scr_dir + "/marker.dir " + scr_dir + "/marker2.dir" )
...
if myproc == 0 :
   ...
   print( str(int(tmove)) + " seconds to move non-icond files from scratch to cwd")
   print( str(int(taverage)) + " seconds for all procs to run average.py" )
...
# If you are not using local /tmp on each node, you can simply exit here
...
for i in range(0,nprocs):
   comm.barrier()
   if myproc == i and os.path.isdir( scr_dir + "/marker2.dir" ) :
      print( str(myproc) + " on " + hostname + " is moving and tarring Spectral Density files to cwd")
      os.system( "cd " + scr_dir + "; find -name '*Spectral_density.txt' | tar -rf " + tarfile + " -T -" )
      os.system( "rmdir " + scr_dir + "/marker2.dir" )

comm.barrier()
if myproc == 0:
   print("Proc 0 is gzipping " + tarfile )
   os.system( "gzip " + tarfile )
ttarzip = os.times()[4] - t0

if myproc == 0 :
   print( str(int(ttarzip)) + " seconds to tar and gzip Spectral Density files from scratch to cwd")
comm.barrier()
